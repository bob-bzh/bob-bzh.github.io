...
$.ajax({
  ...
  data: {
    key: 'NDKBZ-CTTC3-6663N-3BV7E-AK37J-RHBL4',
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(112.517479, 35.667171, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里记录你自己的经纬度
...
function showWelcome() {
  // 判断页面是否含有此 元素 if(!a) a有值时  !a为假 不执行
  if (!document.getElementById("welcome-info")) return



  let dist = getDistance(112.517479, 35.667171, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
  let pos = ipLoacation.result.ad_info.nation;
  let ip;
  let posdesc;
  //根据国家、省份、城市信息自定义欢迎语
  ...
  //自定义文本需要放的位置
  document.getElementById("welcome-info").innerHTML = `欢迎来自<span>${pos}</span>的小伙伴，${timeChange}<br>你距离BoBOvO约有<span>${dist}</span>公里，${posdesc}`;
